...
工作台中的“预设/设定”入口是第一个 Mode × Module 纵向切片：粘贴设定后会创建 `mode=quick-setting` 的本地草稿，并复用现有 `preset` dataMode 开始互动。它不会开放其他规划中或 hidden 的模式。
...
- **预设/设定模式**：粘贴设定快速创建草稿，以 `quick-setting` 入口语义复用现有 `preset` 对话管线。
