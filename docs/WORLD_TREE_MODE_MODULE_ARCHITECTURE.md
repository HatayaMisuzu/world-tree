...
本阶段只建立架构骨架和标准化登记。不会把 hidden 模式暴露给用户，不会让 LLM 直接调用所有模块，也不会实现完整玩法或新增 UI 入口。
...
| Mode | Status | Uses |
|---|---|---|
| `quick-setting` | planned | `core.world_container`, `lore.worldbook_trigger`, `core.dynamic_state`, `scene.session`, `narrative.story_template`, `narrative.five_layer_engine`, `audit.narrative_quality` |
...
Registry 负责标准 ID 与 legacy ID 查询、筛选、依赖展开、缺失项保留、module graph 和状态/分类统计。依赖展开按依赖优先排序、去重；缺失依赖进入 `missing` 和 `warnings`，不会抛错终止。

## 6. ModuleLoader

Loader 根据 mode 的 `uses` 生成 graph，或直接加载一组 module ID。本阶段“加载”只表示解析与描述，不会动态 import 所有实现，不会自动执行模块，也不会将 graph 注入 LLM prompt。
