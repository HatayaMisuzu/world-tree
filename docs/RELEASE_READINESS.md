...
| DeepSeek selected real LLM smoke | RECORDED | `/api/llm/test`, `smoke:first-play`, `smoke:user-content-real-llm`, selected key paths, and long-prefix cache probe recorded external/redacted evidence |
...
3. Full browser/manual smoke evidence for major entries beyond the current reachability matrix.
4. Broader product-wide real LLM evidence beyond selected DeepSeek smoke paths.
